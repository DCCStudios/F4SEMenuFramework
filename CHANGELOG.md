...
- **MCM sliders and checkboxes that moved together are fixed.** Some mods reuse the same option label many times (True Damage's per-weapon Damage and Range sliders, Immersive Hunting Overhaul's spoilage checkboxes, Game Configuration Menu's difficulty sliders), and every copy was collapsing into one control. Each control now moves on its own.
- **MCM Categorizer is built in** (Nexus 66311). The category editor runs natively and your categories and order carry over, so you can remove the original MCM Categorizer mod once they import. That also fixes the crash that hit when MCM Settings Manager saved a slot with the Categorizer installed. The editor and the Categorizer page now sit at the top of the MCM list with a wrench icon so they are easy to find, and a setting hides the built-in editor while the original mod is still present.
- **Hotkeys can use Ctrl, Shift, and Alt combos**, on keyboard and gamepad. Binding a key the game already uses (Jump, Reload, Activate) now warns you the same way a mod-vs-mod clash does, and the warning reads your actual rebinds.
- **The Settings menu is fully translatable.** The MCM Compatibility and Gamepad sections and the keybind mode labels used to be hardcoded English; they now read from the language file, so a translation covers the whole menu. Any plugin menu shown through the framework can also be translated by dropping in language files, with no plugin update.
- **The startup banner shows properly.** It waits for the main menu instead of counting down during the load, so it fades in and stays readable instead of flashing past.
- **MCM Settings Manager: slot apply and float precision.** Applying a slot takes one click again for script-backed settings, and float values compare and show at their real precision instead of inflating the changed count.
- **New setting: Disable Keyboard Input While Using Gamepad** (Settings > Gamepad, off by default). While a controller is connected and the menu is open, the menu ignores keyboard input so stray keys cannot hijack navigation. Binding a hotkey still reads the keyboard, and the toggle key and ESC still work.
- **New setting: Pause Menu Button Position** (Settings). Put the F4SE FRAMEWORK row at the top, the bottom, or a slot in between. It applies the next time the pause menu opens.
- **The gamepad menu-toggle no longer closes the menu.** The default is D-pad Up, which also navigates the menu, so pressing up through a list used to slam it shut. Close with B, Start, ESC, or the keyboard toggle instead.
- **Fixed a startup crash on some F4SE builds.** Very old F4SE installs, and F4SE's hide-trampoline flag, do not hand out the hook trampoline; the framework now allocates its own instead of aborting on the first hook.
- **Plugins get reliable input.** Plugins that register input through the framework now receive keyboard and mouse, including the mouse wheel while aiming. This had been silently broken since the Fallout 4 port.
- **Fallout 4 1.11.240 supported.** One DLL still covers old-gen 1.10.163, next-gen 1.10.980/984, and the 1.11.x line, with the matching Address Library installed.
