...
- **Hotkey conflict detection now includes the player's own Fallout 4 controls.** Rebinding a framework/plugin hotkey onto a key the game already uses (Jump, Activate, Reload, …) raises the same confirmation dialog as a mod-vs-mod clash; game entries are listed with a `[Game]` prefix. Read live from `RE::ControlMap` (the `kMainGameplay` context, the one a hotkey fires in), so it reflects the player's actual rebinds, keyboard and gamepad.
- **Chord hotkeys — Ctrl/Shift/Alt modifiers**, keyboard and gamepad. New `RegisterHotkeyWithModifiers` / `RegisterGamepadHotkeyWithModifiers` / `SetHotkeyBindingWithModifiers` / `GetHotkeyModifiers` / `HasHotkeyConflictWithModifiers` exports take a `HotkeyMod` bitmask (`DIK.h`). A modified binding fires only when exactly those modifiers are held; a plain binding still fires regardless, so the feature is fully additive — existing hotkeys, the INI format (an unmodified bind still writes bare `F1`; chords write `CTRL+F1`), and all current call sites are unchanged. A chord never conflicts with an unmodified game control. Registering with an unknown scan code now also logs a warning (the usual cause is a Windows `VK_*` value where a DIK code was meant), backed by a new `constexpr DIK::IsKnownCode` / `GamepadCode::IsKnownCode`.
- **The `AddInputEvent` plugin API now actually dispatches.** Since the F4 port it was dead code: `RegisterInpoutEvent` stored the callback, but the input-queue hook the Skyrim original called `InputEventHandler::Process` from was replaced by the WndProc path and never re-added, so registered callbacks silently never fired (plugins like MagnaScope had to ship their own receiver hook to work around it). The framework now dispatches the game's raw `RE::InputEvent` queue from `BSInputEventReceiver::PerformInputProcessing` vtable hooks. No single receiver carries every device in every state (verified in game against ADS windows): `PlayerCamera`'s receiver (+0x38, the "Bingle method" one UneducatedShooter/MagnaScope hook) has the keyboard in all states but loses the mouse wheel while the player is sighted, and `PlayerControls`' receiver (offset 0, the slot ScrollWheelWeaponSelect hooks) has the mouse/gamepad in all states but no keyboard. So the framework hooks **both** and splits by device — keyboard from PlayerCamera, mouse+gamepad from PlayerControls — with each event dispatched exactly once. `AddInputEvent` thus delivers keyboard and mouse (including the wheel while scoped) reliably. Both are chain-friendly slot-0 hooks that compose with those mods in install order; no Address Library ID is involved. Consuming an event (returning `true`) unlinks it before the game's player controls see it; callbacks are gated off while a blocking framework window is open, mirroring the Hotkey API. The callback registry is now mutex-guarded (registration is cross-thread API surface; dispatch runs on the input thread). The long-dead `ProcessOpenClose` and `TranslateInputEvent` leftovers (and their orphaned scan-code/translation tables in `Input.cpp`) were removed.
- **Named scan-code constants for the Hotkey API** (`DIK.h`): a single canonical table now generates the framework's INI key-name maps (`GetKeyBinding`/`GetKeyName` previously carried two hand-written copies that could drift) and provides `DIK::F1` / `GamepadCode::DpadUp`-style constants. An optional copy ships in `resources/` next to the consumer header, which picks it up via `__has_include` — plugins that don't register hotkeys still need only the one header.
- **Fixed a startup crash ("failed to handle allocation request. allocate size: 14 Free size: 0") for users whose F4SE build does not serve the trampoline interface** (very old OG F4SE installs, or F4SE's per-plugin hide-trampoline compatibility flag). The vendored CommonLib only created its hook trampoline via that interface; when the interface came back null the trampoline was never allocated, and the first hook install aborted the game. The framework now falls back to allocating its own trampoline, and the log records the F4SE version plus which trampoline source was used so future reports are diagnosable from the log alone.
- **New setting: "Disable Keyboard Input While Using Gamepad"** (Settings > Gamepad, default off; `[Gamepad]` `DisableKeyboardInput` in the INI). While a controller is connected and the menu is open, keyboard events are swallowed before ImGui sees them, so stray key input (Steam Input keyboard emulation, remote play, a bumped keyboard) cannot hijack navigation focus or type into fields. Hotkey capture still reads the keyboard normally (bind prompts work unchanged), and the menu toggle key and ESC-to-close stay active.
- **The gamepad menu-toggle button no longer closes the menu while it is open.** The default binding is D-pad Up, which is also how a controller navigates the menu, so pressing up to move through a list slammed the menu shut. The gamepad toggle is now open-only; closing stays with B (back cascade), Start, ESC, and the keyboard toggle key.
- **New setting: "Pause Menu Button Position"** (Settings, below Blur Background; `[General]` `PauseMenuButtonPos` in the INI). Choose where the F4SE FRAMEWORK row lands in the pause menu list: Top (default), 2nd through 5th from the top, or Bottom. Applies the next time the pause menu opens. Placement uses the same mechanism as the real MCM's `iPosition:Main`: the shipped `F4SEFramework.swf` moves the row to the configured slot on ENTER_FRAME, after the game has populated the vanilla rows (the C++ injects run before population and always land at the top, which is why they are kept only for instant appearance; requires the updated SWF alongside the DLL). Rows other mods add afterwards (e.g. MCM's MOD CONFIG) can still shift it by one.
- **Nav tree: single-click expands mods and categories.** Clicking anywhere on a row that only groups pages (a mod with several pages, an MCM Categorizer folder) now toggles it open; previously only the small arrow toggled and clicking the name did nothing. Rows that are themselves a page keep the old behavior (click selects the page, arrow folds).
- **MCM Categorizer support** (Nexus 66311): when the mod is installed and enabled, the translated MCM mod list under "MCM Mod Configs (Legacy)" is grouped into the user's category folders in the user's order, with the same wrap characters (`«...»`) the Flash list uses. Category folders are visually distinct from mods: a folder icon that opens and closes with the fold state, a theme-derived gold accent, and slightly larger text. A `/` inside a category name (e.g. "Controls/Game Settings") shows as `-`; it would otherwise split into nested folders, and slash lookalike glyphs are missing from the game-style fonts. The mod's drag-and-drop category editor (a Flash app inside an MCM "image" control that needs a live Scaleform stage) is replaced by a native recreation on its MCM page: add, rename, delete and reorder categories, move mods in and out, per-category and global A-Z sort. Rows drag-and-drop like the original (drop a mod on a category header to file it, on another row to reorder, drag category headers to reorder the top level); the arrow buttons and a labeled "Move to..." dropdown remain for gamepad and keyboard use. Edits regroup the nav tree immediately, and toggling the mod's Enable/wrap style settings applies when leaving its page. Everything persists to the mod's own three settings (`sCategories` in its m8rQckSer wire format, `sOrderModNames`, and MCM's `sOrder:Main`), so categories stay in sync both ways with the original Flash UI, and entries we don't render (the real MCM's virtual "MCM"/"Hotkey manager" rows, currently uninstalled mods) are preserved verbatim through edits. Corrupt category data disables grouping and makes the editor read-only instead of overwriting it; the codec is validated by the offline host test `swf/test/qcksertest`.
- **MCM Settings Manager page: applying a slot/preset no longer needs a second click for some settings.** Two causes, both around Papyrus-property-backed settings (the kind Workshop Framework and similar script mods use). First, the bulk Apply skipped any setting whose live value had not yet arrived from the Papyrus VM (property reads are asynchronous), so clicking Apply soon after opening the page silently left those settings out; stored values are now written even while the read is still pending. Second, the post-apply refresh could read a property back BEFORE the queued write executed on the VM thread, overwrite the applied value in the UI with the stale pre-write value, and make the row count as changed again; read-backs that do not match a just-applied value are now discarded and retried (bounded), and only accepted once they confirm the write or the retries run out (a mod's own Set handler genuinely rewriting the value logs a warning and shows the live value).
- **MCM Settings Manager page: float settings compare and display at their real precision.** MCM float values (ModSetting, GlobalValue, PropertyValueFloat) are 32-bit floats, but stored preset values are doubles; the old comparison used a 1e-10 tolerance that sat far below float rounding noise (~5e-8), so a preset value of `1.2` was flagged as different from the game's identical `1.2000000476837158` and inflated the "changed" counts. Floats now compare at float32 precision, and the Stored/Current columns print `1.2` instead of the widened-double noise.
- **Plugin hotkey bindings moved out of `F4SEMenuFramework.ini`** into a new `F4SEMenuFramework/PluginHotkeys.ini` (inside the framework's asset folder next to `Fonts/` and `Themes/`, same `[Hotkeys]` section format). MCM translation layer keybinds (`MCM.<Mod>.<id>`) move with them; `Keybinds.json` remains the shared source of truth for MCM binds as before. The main INI ships with the mod, so every framework update used to overwrite the player's rebinds; the new file is user data created at runtime and is never part of the download. Bindings saved by older versions migrate automatically on first run (existing entries in the new file win, and the legacy section is removed afterwards). The framework's own toggle keys (`[General]` `ToggleKey` / `ToggleKeyGamePad`) intentionally stay in the main INI.
...
- **Translation folder is now the plugin's DLL name**: automatic translation, capture mode, and the single-argument `Translate()` wrappers all resolve the folder from the plugin module itself (`FPGunplayOverhaul.dll` reads and writes `Data/F4SE/Plugins/FPGunplayOverhaul/Languages/`). Previously the `SetSection` display name was preferred, which sent captures and lookups to a differently named folder (`FP Gunplay Overhaul/`) whenever the menu name differs from the DLL name.
